...
bool NTPServer::queryNTPServer(NTPPacket &response)
{
    ...
    NTPPacket request = {};
    request.flags = (NTP_VERSION << NTP_VERSION_OFFSET) |
    ...
    ssize_t receivedBytes = recvfrom(ntpSocket, &serverResponse, sizeof(serverResponse), 0, 
...
{
    NTPPacket response = {};
    response.flags = (NTP_VERSION << NTP_VERSION_OFFSET) |
