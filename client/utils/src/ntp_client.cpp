...
void NTPClient::NTPClientProcedure() {
  ...
  while (m_isSynchronizing) {
    ...
    request.transmitTime.seconds =
        htonl(unixToNTPTime(std::chrono::system_clock::to_time_t(
    ...
    if (sendto(ntpSocket, &request, sizeof(request), 0,
    ...
    if (recvfrom(ntpSocket, &serverResponse, sizeof(serverResponse), 0, nullptr,
                 nullptr) <= 0) {
      close(ntpSocket);
