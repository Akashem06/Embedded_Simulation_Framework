...
SPI::SPI(Payload &data) { m_spiDatagram = data; }

std::string SPI::serialize(const CommandCode &commandCode) const {
  std::string serializedData;

  serializeInteger<uint8_t>(serializedData,
                            static_cast<uint8_t>(m_spiDatagram.spiPort));
  serializeInteger(serializedData,
                   static_cast<uint16_t>(m_spiDatagram.bufferLength));
  serializedData.append(reinterpret_cast<const char *>(m_spiDatagram.buffer),
...
void SPI::deserialize(std::string &spiDatagramPayload) {
  ...
  m_spiDatagram.spiPort = static_cast<Port>(
      deserializeInteger<uint8_t>(spiDatagramPayload, offset));
  m_spiDatagram.bufferLength =
  ...
  std::memcpy(m_spiDatagram.buffer, spiDatagramPayload.data() + offset,
              m_spiDatagram.bufferLength);
}

void SPI::setSPIPort(const Port &spiPort) { m_spiDatagram.spiPort = spiPort; }
...
size_t SPI::getBufferLength() const { return m_spiDatagram.bufferLength; }

const uint8_t *SPI::getBuffer() const { return m_spiDatagram.buffer; }
