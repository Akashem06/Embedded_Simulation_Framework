...
std::string I2C::serialize(const CommandCode &commandCode) const {
  ...
  serializeInteger<uint8_t>(serializedData,
                            static_cast<uint8_t>(m_i2cDatagram.i2cPort));
  serializeInteger(serializedData,
                   static_cast<uint16_t>(m_i2cDatagram.bufferLength));
  serializedData.append(reinterpret_cast<const char *>(m_i2cDatagram.buffer),
                        m_i2cDatagram.bufferLength);
}

void I2C::deserialize(std::string &i2cDatagramPayload) {
  size_t offset = 0;

  m_i2cDatagram.i2cPort = static_cast<Port>(
      deserializeInteger<uint8_t>(i2cDatagramPayload, offset));
  m_i2cDatagram.bufferLength =
      deserializeInteger<uint16_t>(i2cDatagramPayload, offset);

  if (m_i2cDatagram.bufferLength > MAX_BUFFER_SIZE) {
    throw std::runtime_error(
        "Deserialized I2C buffer length exceeds maximum allowed size");
  }

  std::memcpy(m_i2cDatagram.buffer, i2cDatagramPayload.data() + offset,
              m_i2cDatagram.bufferLength);
}
