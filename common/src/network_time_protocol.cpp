...
void dumpNTPPacketData(const NTPPacket packet)
{
    ...
    switch(packet.stratum) {
        ...
        case 1: std::cout << " (Primary Reference)"; break;
        case 2 ... 15: std::cout << " (Secondary Reference)"; break;
        case 16 ... 255: std::cout << " (Reserved)"; break;
    }
    std::cout << std::endl;
    ...
    std::cout << "Poll Interval:  " << static_cast<int>(packet.poll) 
              << " (2^" << static_cast<int>(packet.poll) << " seconds)" << std::endl;
    
    std::cout << "Precision:      " << static_cast<int>(packet.precision) 
              << " (2^" << static_cast<int>(packet.precision) << " seconds)" << std::endl;

    std::cout << "Reference ID:   ";
    char refIdStr[5];
    memcpy(refIdStr, &packet.referenceId, 4);
    refIdStr[4] = '\0';
    std::cout << refIdStr << " (Raw: 0x" 
              << std::hex << packet.referenceId << std::dec << ")" << std::endl;

    std::cout << "Root Delay:     " << (packet.rootDelay / 65536.0) 
              << " seconds" << std::endl;
